...
\ifsplitfigs
  \begin{subfigure}{0.40\linewidth}\centering
    ...
    \caption{}\label{fig:f7a}
  \end{subfigure}\hfill
  \begin{subfigure}{0.59\linewidth}\centering
    \includegraphics[width=\linewidth]{Figure7b.pdf}
    \caption{}\label{fig:f7b}
  \end{subfigure}
\else
  \includegraphics[width=\linewidth]{Figure7.pdf}
\fi
\caption{\textbf{Single-model initial-condition large-ensemble (SMILE) robustness of the mechanism classification.}
\textbf{(a)} F$_{\mathrm{ovS}}$ velocity-share ($f_v$) vs salinity-share ($f_s$) for all 50 initial-condition members of the MPI-ESM1-2-LR CMIP6 contribution (orange crosses), overlaid on the multi-model CMIP6 weakening ensemble (faded coloured circles). The black diamond marks the SMILE ensemble mean. Vertical and horizontal dotted lines mark the $f_v=60\%$ and $f_s=60\%$ classification thresholds; dashed grey line: $f_v+f_s=100\%$. All 50 SMILE members fall in the velocity-dominant quadrant (mean $f_v=+93\%$, $\sigma=15\%$, minimum $f_v=+73\%$); the 60\% threshold sits $2.2\sigma$ below the cluster mean.
\textbf{(b)} AMOC time series at 26.5°N (max of $\mathtt{msftmz}$ below 500~m, the standard RAPID-equivalent definition) for all 50 SMILE members, 1850-2100, hist+ssp585. Orange spaghetti: per-member trajectories. Dark-orange line: ensemble mean. Orange envelope: $\pm 1\sigma$ across members. Vertical dashed line: hist$\rightarrow$ssp585 transition (2014). Grey vertical bands: the 1950-1980 baseline and 2081-2100 forced windows used for percent-weakening. Right inset: histogram of per-member weakening 2081-2100 vs 1950-1980, with the ensemble-mean horizontal line. 1950-1980 ensemble-mean AMOC = $+18.8\pm 0.4$~Sv (1$\sigma$); 2081-2100 ensemble-mean AMOC = $+14.4\pm 0.3$~Sv; per-member weakening = $+23.3\%\pm 2.4\%$ (1$\sigma$), range $[+18\%,+29\%]$.}
\label{fig:smile_robustness}
\end{figure}
...
All analyses are implemented in Python using NumPy, SciPy, xarray, pandas and matplotlib; CMIP6 access uses \texttt{intake-esm} and \texttt{pangeo-cmip6}. The full analysis code, including the F$_{\mathrm{ovS}}$ decomposition kernel, is openly available (see Data and Code Availability).
