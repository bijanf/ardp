...
F$_{\mathrm{ovS}}$ velocity-share vs salinity-share for all 50 initial-condition members of the MPI-ESM1-2-LR CMIP6 contribution (downloaded via DKRZ ESGF), overlaid on the multi-model CMIP6 weakening ensemble (faded background, 12 models). The full ensemble shares identical model physics (POP-family ocean, ECHAM6.3 atmosphere, common parameterizations -- see Supp. Table~\ref{tab:smile_components}); the only inter-member difference is the phase of internal variability at simulation start. If the binary mechanism class were dominated by internal variability rather than structural physics, members would scatter across the plane. They do not: \textbf{50/50 members cluster in the velocity-dominant quadrant}, with $f_v$ mean = $+93\%$, $\sigma = 15\%$, minimum = $73\%$. The classification threshold at $60\%$ is $2.2\sigma$ below the cluster mean; under a Gaussian assumption the probability that a hypothetical 51st member would fall below threshold is $<2\%$. The ensemble-mean $\Delta F_{ovS}$ is $-39.6$~mSv with internal-variability spread $\pm 7.2$~mSv (1$\sigma$) -- so internal variability fails to flip the class even at $\pm 3\sigma$ of the forced signal. This rules out the small-N + internal-variability concern (Supplementary Fig.~S6): MPI-ESM1-2-LR's centennial mechanism class is a structural property of the coupled-response physics, not a phasing of internal decadal variability.}
\label{fig:S7}
\end{figure}
\fi
