...
\centering
\small
\caption{Ocean and atmospheric model components of the 12 weakening CMIP6 models, grouped by mechanism class. Sources: each model's primary CMIP6 description paper (DOIs in references). The table is provided to allow reviewers to audit which physical infrastructure underlies each cluster; it is not used to make a causal attribution.}
\label{tab:smile_components}
\begin{tabular}{l l l l l}
\toprule
\textbf{Class} & \textbf{Model} & \textbf{Ocean component} & \textbf{Ocean res.} & \textbf{Atmosphere} \\
\midrule
v-dom & CESM2          & POP2          & $1^\circ$    & CAM6 \\
v-dom & FIO-ESM-2-0    & POP2          & $1^\circ$    & CAM5 \\
v-dom & MPI-ESM1-2-LR  & MPIOM (GR15)  & $1.5^\circ$  & ECHAM6.3 \\
v-dom & MPI-ESM1-2-HR  & MPIOM (TP04)  & $0.4^\circ$  & ECHAM6.3 \\
v-dom & NESM3          & NEMO 3.4      & $1^\circ$    & ECHAM6.3 \\
\midrule
s-dom & CNRM-CM6-1       & NEMO 3.6        & $1^\circ$    & ARPEGE 6.3 \\
s-dom & FGOALS-g3        & LICOM3          & $1^\circ$    & GAMIL2 \\
s-dom & GFDL-CM4         & MOM6            & $0.25^\circ$ & AM4 \\
s-dom & HadGEM3-GC31-LL  & NEMO 3.6 (ORCA1)& $1^\circ$    & UM-GA7.1 \\
s-dom & MIROC6           & COCO 4.9        & $1^\circ$    & MIROC AGCM \\
s-dom & UKESM1-0-LL      & NEMO 3.6 (ORCA1)& $1^\circ$    & UM-GA7.1 \\
\midrule
mixed & GISS-E2-1-G    & GISS Ocean    & $1^\circ$    & GISS ModelE \\
\bottomrule
\end{tabular}
\end{table}
...
% =====================================================================
\newif\ifsmilefig
\smilefigfalse  % \smilefigtrue when diagSMILE.pdf is ready
\ifsmilefig
\begin{figure}[htbp]
\centering
...
F$_{\mathrm{ovS}}$ velocity-share vs salinity-share for every available initial-condition member of the MPI-ESM1-2-LR Grand Ensemble (50 members on ESGF, of which 10 are also on Pangeo as a cross-check), overlaid on the multi-model CMIP6 weakening ensemble (faded background, 12 models). Each member of the SMILE shares identical model physics; if the binary mechanism class were primarily driven by internal variability rather than structural physics, members would scatter across the plane. They do not: the ensemble clusters tightly in the velocity-dominant quadrant (mean $f_v$ and 1$\sigma$ spread reported in the inset), confirming that MPI-ESM1-2-LR's centennial mechanism class is a structural property of the model and not a phasing of internal decadal variability. This addresses the small-N reviewer concern (Supplementary Fig.~S6): the binary classification used to construct the headline 10-percentage-point gap is structurally meaningful, even though the multi-model ensemble correlation does not yet pass a 5\%-level significance test.}
\label{fig:S7}
\end{figure}
\fi
