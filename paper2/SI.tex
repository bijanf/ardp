...
weakening (Main Fig.~2b,c) is restricted to the 16 models with available
\texttt{msftmz} fields, leaving $n=13$ forced-weakening models in the
gap-quantification subset.
...
the headline mechanism scatter (Main Fig.~2a) to a 25-model footing, but
the AMOC26N (\texttt{msftmz}) field needed for the projection figures
(Main Figs.~2b,c, 3, 6a,b) is currently available only for the original
sixteen-model core; the nine newly-added models will be added to the
projection ensemble in a future revision once their \texttt{msftmz}
...
\item \emph{Mechanism-conditional AMOC weakening} (Main Fig.~2b,c): restricted to the 16 models with available AMOC26N, of which 13 are forced-weakening. Class breakdown on this subset: $n_v = 5$, $n_s = 6$, mixed $= 2$.
\item \emph{Bistable-only subset} (Main Fig.~3): 6 models are bistable and forced-weakening (CNRM-CM6-1, MIROC6, CIESM, MRI-ESM2-0 salinity-dominant; MPI-ESM1-2-HR, NESM3 velocity-dominant). AMOC26N is available for 4 of them, so the violins in Main Fig.~3b show $n_s = 2$, $n_v = 2$.
...
\item \emph{Lead-lag cross-correlation and emergent constraint regression} (Main Fig.~6a,b): \textbf{16 models} -- the original sixteen-model core. The nine v4.3 additions are excluded from these diagnostics because they require both $F_{\mathrm{ovS}}$ \emph{and} AMOC26N annual time series, and AMOC26N is not yet computed for the new models.
\end{itemize}
