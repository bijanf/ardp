...
\title{\textbf{\Large The Atlantic is freshening for opposing reasons --- and only one is consistent with the salt-advection feedback
}}
...
\begin{abstract}
Climate models project Atlantic meridional overturning circulation (AMOC) weakening under continued warming, but the short observational record forces reliance on ocean reanalyses, and these reanalyses partition the recent Atlantic freshening into opposite kinematic components. Four widely used reanalyses place the Atlantic in the bistable regime indicated by $F_{\mathrm{ovS}}<0$, yet split into two camps when the freshening is decomposed: one attributes it to changes in ocean currents (a velocity-share partition), the other to a redistribution of salt (a salinity-share partition consistent with an active salt-advection feedback). The same split appears across CMIP6 models: the salinity-dominant subset projects roughly 10 percentage points more 2100 weakening than the velocity-dominant subset (Spearman $\rho = -0.46$, $p = 0.14$, $n = 12$). This mechanism-conditional uncertainty is not captured by pooled emergent constraints. The decomposition is a kinematic identity, not a dynamical attribution; interpreting any projection requires first disambiguating which mechanism operates, motivating targeted basin-scale Atlantic salinity observations.
\end{abstract}
...
Our reading of the velocity-dominant class as ``less consistent with imminent salt-advection-feedback activation'' must be reconciled with \citet{vanWestenDijkstra2024}, who argue that CMIP6 models systematically carry a positive $F_{\mathrm{ovS}}$ bias and therefore \emph{underestimate} the proximity to a tipping point. Their bias correction shifts a substantial fraction of the multi-model ensemble into the bistable indicator regime ($F_{\mathrm{ovS}}<0$); applied to our partition, the correction would not change the velocity-vs-salinity dichotomy itself (which uses share-fractions rather than absolute $F_{\mathrm{ovS}}$) but would shift the velocity-dominant subset closer to the bistable boundary. The two readings are therefore complementary rather than contradictory: \citet{vanWestenDijkstra2024} establish that bistability is more widespread than uncorrected CMIP6 suggests, while we show that the projected weakening within the bistable ensemble is mechanism-conditional. Independent of either reading, \citet{Mignac2019} caution that the South Atlantic $F_{\mathrm{ovS}}$ in NEMO-based reanalyses is governed by upper-300\,m zonal salinity gradients and AAIW transport rather than by the basin-integrated overturning strength alone, so single-product readings of $F_{\mathrm{ovS}}$ trends should not be over-interpreted as direct AMOC-stability indicators. Direct in-situ observations support the salinity-share reading: \citet{Volkov2024} report a $+0.05$~PSU per decade upper-300\,m salinification trend at 34.5$^\circ$S from the SAMBA repeat-hydrography record (2009--2023), consistent in sign with GLORYS12V1's salinity-share-dominated decomposition and inconsistent with ORAS5's velocity-share interpretation. The companion observational study \citep{FallahInReview} reaches a complementary conclusion using GLORYS12V1: the salt-advection feedback is detectably active in the present-day Atlantic. The mechanism-conditional projection uncertainty reported here therefore arises from a real reanalysis ambiguity, not from purely model-internal disagreement.

\subsection*{Limitations}

The interpretation above is subject to several caveats that should temper any single-projection narrative. First, the $F_{\mathrm{ovS}}$ decomposition is a kinematic identity, not a dynamical attribution: a model's velocity-share or salinity-share quantifies how the change in freshwater transport partitions across two snapshots, but does not isolate the causative forcing (wind stress, surface buoyancy, sea-ice melt) that produced the change. Calling a model ``wind-driven'' or ``buoyancy-driven'' therefore relies on the literature reading of \citet{Asbjornsen2023}, \citet{Levang2020}, and \citet{Baker2023, Baker2025}, not on a direct momentum/buoyancy decomposition of the 15 CMIP6 models analysed here. Second, $F_{\mathrm{ovS}}$ is diagnosed at 34.5$^\circ$S whereas the AMOC index is at 26.5$^\circ$N. The two sections are separated by $\sim$6{,}000~km of equatorial-tropical Atlantic in which the upper-limb salinity is modified by upwelling, NBC retroflection, and lateral mixing with NADW outflow; the lead-lag relation in Figure~\ref{fig:f6_diagnostics}\textbf{(a)} is consistent with\,---\,but does not prove\,---\,the southern-boundary signal propagating northward as a leading-order predictor, and we do not close the cross-latitude budget. Third, the 10-percentage-point class median gap is computed over $n=12$ forced-weakening CMIP6 models with Spearman $\rho = -0.46$ ($p = 0.14$, two-sided), and the binary class median test on $n = \{5,6\}$ subsets has limited statistical power; the gap is reported as a diagnostic regularity, not a statistically established projection. Fourth, the SMILE robustness test bounds initial-condition variability \emph{within} MPI-ESM1-2-LR; a 50-member ensemble of one model cannot establish that mechanism class is robust across model families, and the centennial-vs-decadal timescale check (Supplementary Fig.~S7) shows MPI-ESM1-2-LR is itself the model that flips class outright between window choices. Fifth, the classification depends on a 60\% velocity-share threshold and on a small $|\Delta F_{\mathrm{ovS}}|$ floor; we report the joint sensitivity in Supplementary Fig.~S8. Sixth, ORAS5 and GLORYS12V1 disagree on the sign of the basin-mean South Atlantic salinity trend over 1993--2025 (ORAS5 freshens, GLORYS12V1 salinifies); the opposing-mechanism partition we report can therefore equivalently be read as a sign disagreement on the underlying salinity field.
