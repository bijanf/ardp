...
\textbf{(c)} Projected weakening by 2100 on the AMOC26N subset (n=13 forced-weakening): salinity-dominant median 52\%, velocity-dominant median 42\% -- a $\sim10$-point gap. Red horizontal line and shaded band: observational constraint from \citet{Portmann2026} ($51\pm8\%$).}
...
Same as Figure~\ref{fig:cmip6_projection}(c) but restricted to models that are bistable at baseline (\(F_{\mathrm{ovS}}<0\)). Panel~(a) shows the full ensemble for reference; panel~(b) shows only the bistable-baseline subset. Six models are bistable and forced-weakening (4 salinity-dominant: CNRM-CM6-1, MIROC6, CIESM, MRI-ESM2-0; 2 velocity-dominant: MPI-ESM1-2-HR, NESM3); AMOC26N is available for 4 of them, on which the median weakening is 52\% for salinity-dominant (\(n=2\): CNRM-CM6-1, MIROC6) and 42\% for velocity-dominant (\(n=2\): MPI-ESM1-2-HR, NESM3). Red horizontal line and shaded band: \citet{Portmann2026} constraint.}
...
This mechanistic divergence persists across CMIP6 forced-weakening simulations \citep{Sgubin2017, IPCC_AR6_Chapter9}. The models partition into salinity-dominant and velocity-dominant subsets that bracket the reanalysis end-members; in the 25-model decomposition ensemble assembled from federated ESGF and Pangeo native-grid archives, 20 forced-weakening models split 9 salinity-dominant, 8 velocity-dominant, and 3 mixed (Methods, Supplementary Information). Conditioning future AMOC trajectories on the underlying mechanism class reveals a 9.6-percentage-point gap on the AMOC26N subset (Spearman $\rho = -0.46$, $p = 0.14$, $n = 13$ forced-weakening models with available AMOC26N output; non-parametric bootstrap 95\% CI on the gap = $[-13, +23]$~pp, $P(\mathrm{gap}\leq 0) = 7\%$; Supplementary Fig.~S9): salinity-dominant models project a median 2100 weakening of 52\%, aligning with the emergent-constraint estimate of \citet{Portmann2026}, while velocity-dominant models project a milder 42\% decline. The gap is directionally robust but is not statistically distinguishable from zero at the 5\% level given the $n_v = 5$, $n_s = 6$ subset sizes; we therefore report it as a diagnostic regularity that motivates targeted observation rather than as an established projection. Five CMIP6 models (CMCC-CM2-SR5, CMCC-ESM2, CanESM5, EC-Earth3, IPSL-CM6A-LR) project AMOC weakening despite gaining $F_{\mathrm{ovS}}$ and are excluded from this partition; including them drops the continuous $f_v$--weakening Spearman correlation. Current observational constraints implicitly average over the mechanism dimension, systematically underestimating uncertainty\,---\,a vulnerability increasingly recognized across the AMOC literature \citep{RahmstorfCaesar2026, Jackson2025}.
