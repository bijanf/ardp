...
\title{\textbf{\Large The Atlantic is freshening for opposing reasons
}}
...
\begin{abstract}
Climate models consistently project a weakening of the Atlantic Meridional Overturning Circulation (AMOC) as global temperatures rise. Due to limited direct observations, researchers must rely on ocean reanalyses, which currently disagree on the cause of Atlantic freshening. We show that these reanalysis records fall into two distinct categories: one attributes the freshening to changes in ocean currents, while the other emphasizes salt redistribution. This variation acts as a fundamental source of uncertainty in upcoming climate projections. In CMIP6 models, those that emphasize salt redistribution project a 10\% greater decline in AMOC strength by 2100. Because current climate constraints will not account for this division, it remains unclear which scenario is more likely. Our results show the vital need for targeted, basin-scale salinity observations to determine the underlying mechanism and improve the reliability of climate projections.
\end{abstract}

\linenumbers

The Atlantic Meridional Overturning Circulation (AMOC) transports heat and salt northward and plays a central role in global climate \citep{Weijer2019, BuckleyMarshall2016, IPCC_AR6_Chapter9}. A sustained weakening would carry severe ecological and societal consequences \citep{Rahmstorf1996, vanWesten2024, ManabeStouffer1988}. Reconstructions suggest the AMOC has declined by $\sim$15\% since the mid-twentieth century \citep{Caesar2018, Caesar2021, RahmstorfCaesar2026}, and statistical early-warning signals raise concerns about future stability \citep{Boers2021, Ditlevsen2023}, even as IPCC AR6 retains low confidence in an abrupt 21st-century collapse \citep{IPCC_AR6_Chapter9}. Because continuous in-situ measurements span only two decades at 26.5\textdegree N (RAPID) \citep{McCarthy2015, Smeed2018} and one decade in the subpolar region (OSNAP) \citep{Lozier2019, FrajkaWilliams2019}, physical constraints on future changes rely heavily on model projections and ocean reanalyses \citep{Karspeck2017, Jackson2019}\,---\,each of which can yield divergent answers, raising the question of how much confidence to place in any single projection.

A widely used metric for AMOC tipping-point risk is the overturning freshwater transport at 34.5\textdegree S, $F_{\mathrm{ovS}}$, defined as the depth integral of the product of the zonally-averaged meridional velocity (with the barotropic component removed) and the salinity anomaly relative to $S_0 = 35$~PSU \citep{deVriesWeber2005, HofmannRahmstorf2009}. A negative $F_{\mathrm{ovS}}$ implies that the AMOC exports freshwater, so a slowing overturning leaves a net salt accumulation in the North Atlantic; this basin-scale salinification densifies the upper water column and restabilizes the sinking branch explained by Stommel's salt-advection feedback \citep{Stommel1961}. When this feedback kicks in, the system resides in a \emph{bistable} regime \citep{Rahmstorf1996, Dijkstra2007, vanWesten2024, Cimatoribus2014}; a positive $F_{\mathrm{ovS}}$ implies a single stable regime. $F_{\mathrm{ovS}}<0$ is widely used as an indicator of bistability, although it is not a sufficient condition when gyre-scale freshwater transports are comparable to the overturning component \citep{Gent2018}.

Observational and ocean reanalyses datasets show a negative mean \(F_{\mathrm{ovS}}\), placing the present-day Atlantic firmly within the bistable regime \citep{ArumiPlanas2024, Portmann2026}. Yet, recent observed declines in \(F_{\mathrm{ovS}}\) introduce a critical physical ambiguity: the trend could emerge from two fundamentally different mechanisms. Wind-driven alterations in the boundary current system ("velocity-driven" changes) can reduce freshwater transport without producing profound regime changes, as these dynamics are largely reversible \citep{Chemke2021}. Conversely, the decline could trace directly to a basin-scale buoyancy perturbation ("salinity-driven" changes), the kinematic fingerprint expected from an active salt-advection feedback \citep{deVriesWeber2005, Drijfhout2011, vanWesten2024, Liu2017, Sgubin2017, FallahInReview}. Because current measurement-based constraints focus strictly on the magnitude of the \(F_{\mathrm{ovS}}\) decline, they obscure which physical mechanism is operating. This gap compromises our ability to determine if the freshening signals a future tipping point or just a temporary fluctuation, directly weakening trust in model-based AMOC projections.

To isolate the different mechanisms, we decompose the recent change in \(F_{\mathrm{ovS}}\) into a velocity-driven component (\(\Delta F_v\)), a salinity-driven component (\(\Delta F_s\)), and their cross term (\(\Delta F_{\mathrm{cross}}\)). We evaluate this decomposition across four independent, widely used ocean reanalyses (ORAS5 \citep{Zuo2019}, GLORYS12V1 \citep{Lellouche2021}, SODA 3.15.2 \citep{Carton2018}, and ECCO-V4r4 \citep{Forget2015}). These reanalysis span across the major data-assimilation paradigms and match the canonical ensemble used in recent emergent-constraint studies \citep{Portmann2026}. We use 25 CMIP6 models in our analysis. The full per-analysis bookkeeping is detailed in the Supplementary Information, along with the decomposition methodology and statistical tests. Here, we show that two leading reanalyses partition the observed Atlantic freshening into entirely opposite kinematic components, with implications for future AMOC trajectories. This fundamental disagreement creates a blind spot for existing emergent constraints. Because these constraints do not account for such qualitative differences in drivers, they fail to resolve a 12-percentage-point divergence in future projections. By 2100, the median AMOC weakening differs significantly depending on whether a model is salinity- or velocity-dominant (Spearman $\rho = -0.56$, $p = 0.018$, $n = 17$ forced-weakening models with AMOC at 26.5\textdegree N).
...
\textbf{(a)} Long-term \(F_{\mathrm{ovS}}\) time series at 34.5°S. The shaded region (\(F_{\mathrm{ovS}}<0\)) indicates the bistable regime of the salt-advection feedback, where a weakening AMOC would increase North Atlantic salinity. 
Diamonds denote direct hydrography estimates. The change \(\Delta F_{\mathrm{ovS}}\) is split into a velocity-driven part \(\Delta F_v\) (due to circulation changes only), a salinity-driven part \(\Delta F_s\) (due to salinity redistribution only), and a cross term \(\Delta F_{\mathrm{cross}}\) (correlations). \textbf{(c)} Vertical profiles of the integrands.}
\label{fig:reanalysis_mechanism}
\end{figure}

\paragraph*{Reanalyses disagree on the mechanism driving AMOC-relevant freshening}

We apply an exact decomposition that separates the change in \(F_{\mathrm{ovS}}\) into a velocity-driven part (\(\Delta F_v\)), a salinity-driven part (\(\Delta F_s\)), and a cross term (\(\Delta F_{\mathrm{cross}}\)). \(\Delta F_v\) isolates changes that would occur if only the circulation structure changed while salinity remained fixed. \(\Delta F_s\) separates changes that would occur if only the salinity distribution changed while circulation remained fixed. The cross term accounts for correlations between velocity and salinity anomalies. Applying this decomposition to four independent ocean reanalyses (ORAS5 \citep{Zuo2019}, GLORYS12V1 \citep{Lellouche2021}, SODA 3.15.2 \citep{Carton2018}, ECCO-V4r4 \citep{Forget2015}) yields a previously undocumented finding.

Figure~\ref{fig:reanalysis_mechanism} presents this result. Panel~(a) confirms that all four reanalyses have negative long-term means (from \(-0.009\) to \(-0.116\) Sv), placing the Atlantic in the bistable regime. Trends, however, differ: ORAS5 and GLORYS12V1 show significant declines (\(-1.4\) and \(-4.4\) mSv yr\(^{-1}\), respectively), whereas SODA and ECCO exhibit nearly flat trends. Panel~(b) resolves that inconsistency. It reveals that ORAS5 attributes 89\% of its decline to \(\Delta F_v\) (velocity-driven), whereas GLORYS12V1 attributes 87\% to \(\Delta F_s\) (salinity-driven). The two most widely used reanalyses point to opposite physical mechanisms for the same observed freshening. The vertical profiles in panel~(c) show that these opposite attributions originate from depth-dependent differences in velocity and salinity anomalies.

This result fundamentally improves our understanding of Atlantic freshening. The mechanism itself, not just the magnitude of the \(F_{\mathrm{ovS}}\) decline, is reanalysis-dependent. Prior knowledge assumed that different reanalyses would agree on the mechanism because they assimilate similar observations. Consequently, any observational constraint that uses reanalysis data to project future AMOC weakening implicitly inherits an invalidated mechanistic assumption. This ambiguity translates directly into substantial uncertainty in near-term AMOC projections, as we demonstrate next using CMIP6 models that naturally split into the same two mechanism classes.

\paragraph*{CMIP6 models mirror the reanalysis split and reveal a 10\% difference in AMOC projections}

The same exact decomposition applied to 20 CMIP6 models that exhibit forced AMOC weakening under SSP5-8.5 (historical 1950--1980 versus 2080--2100) reproduces the mechanistic diversity seen in the reanalyses. As shown in Figure~\ref{fig:cmip6_projection}\textbf{(a)}, nine models are salinity-dominant (salinity share \(>60\%\)), eight are velocity-dominant (velocity share \(>60\%\)), and three are mixed. The two reanalysis end-members i.e., ORAS5 (velocity-dominant) and GLORYS12V1 (salinity-dominant) fall at opposite corners of the CMIP6 cloud. A null test based on 2600 random 30-year segments from piControl runs confirms that these forced signals lie well above internal variability (see Figure~\ref{fig:robustness}\textbf{c}).

The consequence of this mechanistic split for future AMOC weakening is quantified in Figure~\ref{fig:cmip6_projection}\textbf{(b,c)}. AMOC at 26.5\textdegree N is available for 17 of the 20 forced-weakening models; on this subset, partitioning by mechanism class shows the salinity-dominant subset (\(n=7\)) projects a median AMOC weakening of 54\% by 2100 (IQR 48--55\%). In contrast, the velocity-dominant subset (\(n=7\)) projects only 42\% (IQR 32--45\%). The salinity-dominant median is statistically indistinguishable from the observational constraint of \citet{Portmann2026} (51\(\pm\)8\%), whereas the velocity-dominant median is nearly 12 percentage points lower. The continuous Spearman correlation between velocity-share and projected weakening across the same 17 models is significant ($\rho = -0.56$, $p = 0.018$; Supplementary Fig.~S6), independently confirming the binary partition. The bistable-only subset (Figure~\ref{fig:bistable_subset}, $n=5$) is the cleanest like-with-like comparison to the four observational reanalyses, all of which sit in the bistable regime; the persistence of the gap in that subset rules out the well-known CMIP6 fresh bias in the South Atlantic as a confounding factor.

The mechanism split is not an artifact of model resolution. Within our ensemble, the two MPI-ESM1-2 siblings share identical physics but differ in ocean resolution (LR: $\sim$1.5\textdegree, HR: $\sim$0.4\textdegree). After applying the barotropic correction, both are classified as velocity-dominant, ruling out resolution as the sole discriminator. The same barotropic correction has long been recognised as essential in idealised water-hosing intercomparisons \citep{Stouffer2006}, where mass-budget closure across the diagnostic section determines whether the simulated freshwater fingerprint is interpretable as an overturning signal.

% =========================================================================
% Figure 2: CMIP6 validation and the 12-percentage-point projection gap
% =========================================================================
\begin{figure}
\centering
\ifsplitfigs
...
\fi
\caption{\textbf{CMIP6 uncertainty in AMOC weakening.}
\textbf{(a)} Velocity-share vs salinity-share for 25 CMIP6 models (20 forced-weakening: 8 velocity-dominant, 9 salinity-dominant, 3 mixed; 5 increasing). Symbol edge: black = bistable baseline, grey = monostable. Reanalysis products are plotted as diamonds (ORAS5 in dark blue, GLORYS12V1 in dark green) and lie at opposite corners of the CMIP6 cloud. CMIP6 model identities are documented in the Supplementary Information; only the four reanalyses are labelled here for clarity.
\textbf{(b)} Mechanism-conditional historical and future AMOC at $26.5^\circ\mathrm{N}$ for the 16 CMIP6 models with available AMOC26N output, grouped by mechanism: salinity-dominant (light blue), velocity-dominant (orange) and mixed (green). Vertical dashed line marks the historical $\rightarrow$ SSP5-8.5 transition (2014).
\textbf{(c)} Projected weakening by 2100 on the AMOC26N subset (n=17 forced-weakening): salinity-dominant median 54\%, velocity-dominant median 42\% -- a $\sim12$-point gap. Red horizontal line and shaded band: observational constraint from \citet{Portmann2026} ($51\pm8\%$).}
...
Same as Figure~\ref{fig:cmip6_projection}(c) but restricted to models that are bistable at baseline (\(F_{\mathrm{ovS}}<0\)). Panel~(a) shows the full ensemble for reference; panel~(b) shows only the bistable-baseline subset. Six models are bistable and forced-weakening (4 salinity-dominant: CNRM-CM6-1, MIROC6, CIESM, MRI-ESM2-0; 2 velocity-dominant: MPI-ESM1-2-HR, NESM3); AMOC26N is available for 5 of them, on which the median weakening is 54\% for salinity-dominant (\(n=3\): CNRM-CM6-1, MIROC6, MRI-ESM2-0) and 37\% for velocity-dominant (\(n=2\): MPI-ESM1-2-HR, NESM3). Red horizontal line and shaded band: \citet{Portmann2026} constraint.}
\label{fig:bistable_subset}
\end{figure}

\paragraph*{The observed freshening trend is not robust across reanalyses}

The disagreement of mechansims between ORAS5 and GLORYS12V1 is striking, but a deeper question remains: is the decline in \(F_{\mathrm{ovS}}\) itself a robust signal? To test this, we repeated the decomposition using only fully post-Argo windows (2006--2012 versus 2018--2024), thereby removing any possible artifact from the pre-Argo observing-system shock. As shown in Figure~\ref{fig:robustness}(a), GLORYS12V1 remains strongly salinity-dominant (\(+90\%\) salinity share), confirming that its attribution to salt redistribution is not a spurious result of the Argo transition. By contrast, ORAS5 shifts from velocity-dominant (89\% velocity share in the main windows) to mixed (44\% velocity share, 57\% salinity share) in the post-Argo windows. This indicates that part of ORAS5's original velocity-driven signal is likely an artifact of the observing-system shock rather than a forced climate trend.

Even more critically, the only 4D-variational product in our ensemble, ECCO-V4r4, exhibits a near-zero change (\(\Delta F_{\mathrm{ovS}} = -2\) mSv, well below the 10 mSv threshold for mechanism classification). ECCO-V4r4 is produced by a long-window adjoint (4D-variational) optimisation that enforces dynamical self-consistency and near-mass-conservation over the full record by iteratively adjusting initial conditions and forcing. Its lack of trend is the dynamically-consistent answer. This null result directly contradicts the large declines seen in ORAS5 (\(-34\) mSv) and GLORYS12V1 (\(-91\) mSv). The disagreement cannot be dismissed as a smoothing artifact; it must either reflect ECCO's weaker assimilation constraints on interior salinity, or indicate that the ORAS5/GLORYS12V1 trends are partially observing-system artifacts.

Taken together, the robustness tests (Figure~\ref{fig:robustness}(b) and (c)) confirm that the mechanism split is real and not a noise artifact, but they also reveal a more profound uncertainty: we cannot be certain that the \(F_{\mathrm{ovS}}\) trend itself is a forced climate signal. While GLORYS12V1's salinity-dominance survives all checks, ORAS5's velocity attribution is partly artefactual, and ECCO's null trend challenges whether any secular decline has actually occurred. Therefore, the community's confidence in AMOC weakening projections must be tempered not only by the mechanism ambiguity but also by the possibility that the observational fingerprint itself is reanalysis-dependent.
...
\textbf{(b)} CMIP6 emergent regression of $F_{\mathrm{ovS}}$ at 34.5°S (2000-2024) against $\Delta\mathrm{AMOC}_{26^\circ\mathrm{N}}$ over the 2030-2040 forecast window ($n=21$ models). Slope $a = +0.49$, intercept $b = -1.79$, $R^2 = 0.01$, $\sigma_\mathrm{res} = 0.95$~Sv. Blue band: observational $F_{\mathrm{ovS}}$ ($-0.064\pm0.049$~Sv) propagated through the regression; the resulting constrained forecast $\Delta\mathrm{AMOC} = -1.82$~Sv (95\% CI $[-3.68, +0.04]$) is essentially identical to the unconstrained CMIP6 mean. Partitioning the regression by mechanism class does not recover predictive power either (within-class $R^2 \le 0.10$; Supplementary Fig.~S5). The 12-percentage-point AMOC-weakening gap visible in Figure~\ref{fig:cmip6_projection}\textbf{(c)} therefore arises from the mechanism-class partition itself rather than from any direct $F_{\mathrm{ovS}}$--AMOC regression.
...
A reasonable concern with the binary mechanism classification of single-realisation CMIP6 models is that the assigned class -- velocity-dominant vs salinity-dominant -- might be partly an artefact of the phase of internal variability sampled by the \texttt{r1i1p1f1} realisation, rather than a structural property of the model. We address this directly using the 50-member MPI-ESM1-2-LR Grand Ensemble downloaded via DKRZ ESGF: every member shares identical model code, parameterizations and forcing, and only the initial-condition perturbation differs. As shown in Figure~\ref{fig:f6_diagnostics}, all 50 members fall in the velocity-dominant quadrant of the F$_{\mathrm{ovS}}$ decomposition plane (panel~\textbf{(c)}; ensemble-mean $f_v=+93\%$, standard deviation $\sigma=15\%$; the $60\%$ classification threshold sits $2.2\sigma$ below the cluster mean), and the AMOC weakening at 26.5°N (panel~\textbf{(d)}) clusters tightly across all 50 members (ensemble-mean weakening $+23.3\%$ by 2100, $\sigma=2.4$ percentage points; range $[+18\%,+29\%]$). The internal-variability spread on the projected weakening is therefore $\sim$5$\times$ smaller than the multi-model 12-percentage-point gap reported in Figure~\ref{fig:cmip6_projection}\textbf{(c)}, confirming that the mechanism-class assignment is set by structural physics -- chiefly the model's coupled wind-driven Southern-Hemisphere response and NADW-formation sensitivity (Discussion) -- rather than by a particular phasing of decadal variability.

\section*{Discussion}

Our findings characterise a fundamental ambiguity in ocean reanalysis representations of Atlantic freshening. All four evaluated products identify a negative \(F_{\mathrm{ovS}}\), placing the present-day Atlantic in the bistable indicator regime, yet they partition the recent decline into opposite kinematic components. ORAS5's decomposition is dominated by circulation shifts (velocity-share), whereas GLORYS12V1's is dominated by a basin-scale salt redistribution (salinity-share) consistent with the kinematic fingerprint of an active salt-advection feedback. Wind-forced velocity trends are largely reversible \citep{Chemke2021} and place no diagnostic constraint on basin-wide salt-advection-feedback activation. A salinity-share-dominated decomposition is consistent with active salt-advection feedback at 34.5$^\circ$S, but mapping that southern-boundary diagnostic to a 21st-century North Atlantic convective collapse risk would require a quantitative cross-latitude budget closure between 34.5$^\circ$S and the subpolar gyre that the present analysis does not provide.

This mechanistic divergence persists across CMIP6 forced-weakening simulations \citep{Sgubin2017, IPCC_AR6_Chapter9}. The models partition into salinity-dominant and velocity-dominant subsets that bracket the reanalysis end-members; in the 25-model decomposition ensemble assembled from federated ESGF and Pangeo native-grid archives, 20 forced-weakening models split 9 salinity-dominant, 8 velocity-dominant, and 3 mixed (Methods, Supplementary Information). Conditioning future AMOC trajectories on the underlying mechanism class reveals an 11.6-percentage-point gap on the AMOC26N subset (Spearman $\rho = -0.56$, $p = 0.018$, $n = 17$ forced-weakening models with available AMOC26N output; non-parametric bootstrap 95\% CI on the gap = $[-0.7, +24.7]$~pp, $P(\mathrm{gap}\leq 0) = 2.6\%$; Supplementary Fig.~S9): salinity-dominant models project a median 2100 weakening of 54\%, aligning with the emergent-constraint estimate of \citet{Portmann2026}, while velocity-dominant models project a milder 42\% decline. With $n_v = n_s = 7$ the bootstrap interval excludes zero by design under standard percentile rules but admits a small upper tail close to zero; we therefore report the gap as a robust diagnostic regularity that survives expansion of the ensemble to 25 models. Five CMIP6 models (CMCC-CM2-SR5, CMCC-ESM2, CanESM5, EC-Earth3, IPSL-CM6A-LR) project AMOC weakening despite gaining $F_{\mathrm{ovS}}$ and are excluded from this partition; their inclusion would dilute the continuous $f_v$--weakening Spearman correlation. Current observational constraints implicitly average over the mechanism dimension, systematically underestimating uncertainty\,---\,a vulnerability increasingly recognized across the AMOC literature \citep{RahmstorfCaesar2026, Jackson2025}.

Robustness tests support this division. GLORYS12V1's salinity-dominant signature is consistent across the full record and the post-Argo subwindow, suggesting it is not an observing-system artefact; ORAS5's velocity-dominant signal, by contrast, deteriorates into a mixed state during the post-Argo era, consistent with residual data-assimilation effects in its 3D-Var FGAT scheme. ECCO-V4r4 (the only 4D-variational adjoint estimate) exhibits a near-zero trend, rendering the existence of a forced secular freshening trend observationally uncertain.

The velocity-vs-salinity dichotomy at 34.5$^\circ$S parallels the wind-vs-buoyancy split documented in recent CMIP6 AMOC-attribution studies: \citet{Asbjornsen2023} attribute roughly one-third of projected Gulf Stream weakening at 26.5$^\circ$N to wind-stress curl changes; \citet{Levang2020} find geostrophic AMOC decline at 40$^\circ$N dominated by 1000--2000\,m temperature anomalies; \citet{Baker2023} link the 29--61\% projected weakening spread to the Indo-Pacific--Southern-Ocean overturning return-pathway partition; and \citet{Baker2025} argue Southern-Ocean wind-driven upwelling sustains a weakened-but-non-collapsed AMOC under climate-extreme scenarios. The velocity-dominant CMIP6 subset (ACCESS-ESM1-5, CESM2, CESM2-WACCM, FIO-ESM-2-0, MPI-ESM1-2-LR, MPI-ESM1-2-HR, NESM3, TaiESM1) is most naturally read as the family in which the Southern-Hemisphere wind-driven response dominates the $F_{\mathrm{ovS}}$ change, consistent with ORAS5's western-boundary-current velocity anomaly (Fig.~5\textbf{(a)}); the salinity-dominant subset (BCC-CSM2-MR, CIESM, CNRM-CM6-1, FGOALS-g3, GFDL-CM4, HadGEM3-GC31-LL, MIROC6, MRI-ESM2-0, UKESM1-0-LL) maps to models in which the buoyancy-driven NADW reduction propagates south as the salt-pile-up signature predicted by salt-advection-feedback theory \citep{Stommel1961, deVriesWeber2005}, visible in GLORYS12V1's spatial pattern (Fig.~5\textbf{(d)}). The 50-member MPI-ESM1-2-LR Grand Ensemble (Figure~\ref{fig:f6_diagnostics}\textbf{(c,d)}) shows MPI's velocity-dominant class is robust to initial-condition variability (50/50 members in the v-quadrant; $f_v$ mean $+93\%$, $\sigma = 15\%$). This bounds within-model aliasing for MPI-ESM1-2-LR but does not establish cross-family class robustness, and MPI-ESM1-2-LR is itself the model that flips class between centennial and decadal $F_{\mathrm{ovS}}$ windows (Supplementary Fig.~S7).

This reading must be reconciled with \citet{vanWestenDijkstra2024}, who argue that CMIP6 carries a positive $F_{\mathrm{ovS}}$ bias and therefore underestimates tipping proximity. Their bias correction shifts more models into the bistable indicator regime but, because our partition uses share-fractions rather than absolute $F_{\mathrm{ovS}}$, leaves the velocity-vs-salinity dichotomy intact: the two readings are complementary, with bistability more widespread than uncorrected CMIP6 suggests and the projected weakening within the bistable ensemble mechanism-conditional. \citet{Mignac2019} additionally caution that South Atlantic $F_{\mathrm{ovS}}$ in NEMO-based reanalyses is governed by upper-300\,m zonal salinity gradients and AAIW transport rather than the basin-integrated overturning alone. In-situ observations favour the salinity-share reading: \citet{Volkov2024} report $+0.05$~PSU per decade upper-300\,m salinification at 34.5$^\circ$S from the SAMBA repeat-hydrography record (2009--2023), consistent with GLORYS12V1 and inconsistent with ORAS5; and the companion observational study \citep{FallahInReview} reaches a complementary conclusion using GLORYS12V1.

\subsection*{Limitations}

Several caveats temper any single-projection narrative. (i) The $F_{\mathrm{ovS}}$ decomposition is a kinematic identity, not a dynamical attribution: it partitions the change but does not isolate the causative forcing, so the wind- vs buoyancy-driven labels rely on the cited attribution literature \citep{Asbjornsen2023, Levang2020, Baker2023, Baker2025} rather than on a direct momentum/buoyancy decomposition. (ii) $F_{\mathrm{ovS}}$ is diagnosed at 34.5$^\circ$S whereas the AMOC index is at 26.5$^\circ$N, with $\sim$6{,}000~km of equatorial-tropical Atlantic in between; the lead-lag relation in Figure~\ref{fig:f6_diagnostics}\textbf{(a)} is consistent with, but does not prove, the southern-boundary signal as a leading-order predictor. (iii) The class-median gap (Spearman $\rho = -0.56$, $p = 0.018$; bootstrap 95\% CI $[-0.7, +24.7]$~pp) is now significant at the 5\% level after expanding the AMOC subset from $n=13$ to $n=17$ via dedicated msftmz extraction. (iv) The SMILE bounds within-model internal-variability for MPI-ESM1-2-LR only and does not establish cross-family robustness; MPI-ESM1-2-LR itself flips class between centennial and decadal windows (Supplementary Fig.~S7). (v) The classification depends on a 60\% threshold and a small $|\Delta F_{\mathrm{ovS}}|$ floor; the joint sensitivity is reported in Supplementary Fig.~S8. (vi) ORAS5 and GLORYS12V1 disagree on the sign of the basin-mean South Atlantic salinity trend over 1993--2025, so the opposing-mechanism partition can equivalently be read as a salinity-trend sign disagreement.

These findings impose a condition on future AMOC constraints: projections calibrated against historical reanalyses must explicitly incorporate the reanalysis-dependent mechanism. The alignment between the salinity-dominant CMIP6 subset and the constraint of \citet{Portmann2026} indicates an implicit physical selection that an explicit metric\,---\,for instance, the zonally-integrated $\Delta S$ anomaly\,---\,would make transparent. Resolving the ambiguity ultimately demands targeted observation: the SAMBA array at 34.5$^\circ$S \citep{Meinen2018, Kersale2020, Volkov2024} convolves velocity and salinity variations, so a basin-wide South Atlantic salinity repeat-section array, parallel to OSNAP \citep{Lozier2019, FrajkaWilliams2019}, would directly arbitrate the reanalysis split documented here.
...
CMIP6 meridional-velocity (\texttt{vo}) and salinity (\texttt{so}) sections at 34.5$^\circ$S are obtained for every available model with coverage of both the \texttt{historical} experiment (required period 1950--1980) and the \texttt{ssp585} experiment (required period 2080--2100). The native grid of each model is used (i.e. no regridding), because regridding distorts the mechanistic decomposition. Twenty-five models satisfy both criteria with accessible section files: ACCESS-CM2, ACCESS-ESM1-5, BCC-CSM2-MR, CESM2, CESM2-WACCM, CIESM, CMCC-CM2-SR5, CMCC-ESM2, CNRM-CM6-1, CanESM5, EC-Earth3, FGOALS-f3-L, FGOALS-g3, FIO-ESM-2-0, GFDL-CM4, GISS-E2-1-G, HadGEM3-GC31-LL, IPSL-CM6A-LR, MIROC6, MPI-ESM1-2-HR, MPI-ESM1-2-LR, MRI-ESM2-0, NESM3, TaiESM1, UKESM1-0-LL. NorESM2-LM and NorESM2-MM are excluded because their ocean component is on isopycnal (density) coordinates and cannot be integrated in depth-space without an additional remap that would confound the decomposition. All models use the \texttt{r1i1p1f1} realisation.
...
\paragraph{Ensemble size used per analysis.} Different downstream analyses retain different subsets of the 25-model decomposition ensemble:

\begin{itemize}
\item \emph{Mechanism decomposition + tiebreaker scatter} (Figs.~1b, 2a of the main text): all 25 models. Five models are flagged \emph{increasing} (CMCC-CM2-SR5, CMCC-ESM2, CanESM5, EC-Earth3, IPSL-CM6A-LR) and excluded from the mechanism-conditional projection because their forced $\Fovs$ change is positive; this leaves \textbf{20 forced-weakening models} (8 velocity-dominant, 9 salinity-dominant, 3 mixed) underpinning the headline mechanism partition.
\item \emph{Bistable-only subset} (Fig.~3): 6 models are bistable and forced-weakening (CNRM-CM6-1, MIROC6, CIESM, MRI-ESM2-0 salinity-dominant; MPI-ESM1-2-HR, NESM3 velocity-dominant). AMOC26N is available for 5 of them (CIESM publishes no streamfunction), so Fig.~3b shows $n_s = 3$, $n_v = 2$.
...
\item \emph{Lead-lag cross-correlation} (Fig.~6a) and \emph{emergent constraint regression} (Fig.~6b): \textbf{21 models}. Both diagnostics require both $F_{\mathrm{ovS}}$ and AMOC26N annual time series; the four models without published \texttt{msftmz}/\texttt{msftyz} (BCC-CSM2-MR, CIESM, EC-Earth3, TaiESM1) are necessarily dropped here even though they participate in the mechanism scatter.
\item \emph{AMOC26N gap quantification} (Fig.~2c): \textbf{17 forced-weakening models} -- the 21-model AMOC subset minus the four \emph{increasing} models retained for emergent-constraint power. v-dominant ($n=7$): ACCESS-ESM1-5, CESM2, CESM2-WACCM, FIO-ESM-2-0, MPI-ESM1-2-HR, MPI-ESM1-2-LR, NESM3. s-dominant ($n=7$): CNRM-CM6-1, FGOALS-g3, GFDL-CM4, HadGEM3-GC31-LL, MIROC6, MRI-ESM2-0, UKESM1-0-LL. mixed ($n=3$): ACCESS-CM2, FGOALS-f3-L, GISS-E2-1-G.
\end{itemize}
