...
% \splitfigstrue   -> include each panel as a separate PDF
...
To resolve this ambiguity, we introduce an exact decomposition that separates the change in \(F_{\mathrm{ovS}}\) into a velocity-driven part (\(\Delta F_v\)), a salinity-driven part (\(\Delta F_s\)), and a cross term (\(\Delta F_{\mathrm{cross}}\)). Using this decomposition, we analyse four independent ocean reanalyses (ORAS5 \citep{Zuo2019}, GLORYS12V1 \citep{Lellouche2021}, SODA 3.15.2 \citep{Carton2018}, ECCO-V4r4 \citep{Forget2015}) -- chosen to span the four mainstream data-assimilation paradigms (3D-Var, reduced-order Kalman filter, ensemble OI, 4D-Var adjoint) and to match the canonical four-product ensemble used by the headline emergent-constraint study \citep{Portmann2026} -- together with 15 CMIP6 climate models. The CMIP6 ensemble size differs across analyses (15 for the mechanism decomposition; 12 of those that exhibit forced AMOC weakening for the mechanism-conditional projection; 13 with accessible \texttt{piControl} for the null test; and 16 -- the 15 plus ACCESS-CM2 -- for the lead-lag and emergent-constraint diagnostics, where the late-period 2080--2100 \texttt{ssp585} requirement does not apply). The full per-analysis bookkeeping is given in the Supplementary Information, along with detailed descriptions of the reanalysis products, model simulations, decomposition methodology, and statistical tests. The key result is that two of the most widely used reanalyses attribute the observed freshening to opposite mechanisms -- with consequential differences for future AMOC weakening. This finding demonstrates that the choice of model or reanalysis fundamentally affects the inferred mechanism, and therefore the reliability of any single projection. The uncertainty is not merely quantitative; it is qualitative, and it translates directly into a \(\sim\)10-percentage-point spread in projected AMOC weakening by 2100.
