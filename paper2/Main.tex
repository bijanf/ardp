...
Same as Figure~\ref{fig:cmip6_projection}(c) but restricted to models that are bistable at baseline (\(F_{\mathrm{ovS}}<0\)). Panel~(a) shows the full ensemble for reference; panel~(b) shows only the bistable-baseline subset. The median AMOC weakening remains 52\% for salinity-dominant models (\(n=4\)) and 42\% for velocity-dominant models (\(n=2\)). Red horizontal line and shaded band: \citet{Portmann2026} constraint.}
