...
\begin{abstract}
Climate models consistently project a weakening of the Atlantic meridional overturning circulation (AMOC) under continued warming, but the short observational record forces reliance on ocean reanalyses, and these reanalyses partition the recent Atlantic freshening into opposite kinematic components. Four widely used reanalyses place the present-day Atlantic in the bistable regime indicated by $F_{\mathrm{ovS}}<0$, yet they split into two opposing camps when the freshening is decomposed: one group attributes it predominantly to changes in ocean currents (a velocity-share partition), the other to a redistribution of salt (a salinity-share partition consistent with the kinematic fingerprint of an active salt-advection feedback). The same mechanistic split appears across CMIP6 models. When future AMOC projections are grouped by mechanism class, the salinity-dominant subset projects roughly 10 percentage points more weakening by 2100 than the velocity-dominant subset (Spearman $\rho = -0.46$, $p = 0.14$, $n = 12$ weakening models). This mechanism-conditional uncertainty is not captured by pooled emergent constraints, which average over the two interpretations. The decomposition is a kinematic identity, not a dynamical attribution; interpreting any single projection therefore requires first disambiguating which mechanism is operating, motivating targeted basin-scale Atlantic salinity observations.
\end{abstract}
...
Analyses of observational data and ocean reanalyses show a negative mean \(F_{\mathrm{ovS}}\), placing the present-day Atlantic firmly within the bistable regime \citep{ArumiPlanas2024, Portmann2026}. Yet, recent observed declines in \(F_{\mathrm{ovS}}\) introduce a critical physical ambiguity: the trend could emerge from two fundamentally different mechanisms. Wind-driven alterations in the boundary current system ("velocity-driven" changes) can reduce freshwater transport without invoking profound regime shifts, as these dynamics are largely reversible \citep{Chemke2021}. Conversely, the decline could trace directly to a basin-scale buoyancy perturbation ("salinity-driven" changes), the kinematic fingerprint expected from an active salt-advection feedback \citep{deVriesWeber2005, Drijfhout2011, vanWesten2024, Liu2017, Sgubin2017}. Because current observational constraints focus strictly on the magnitude of the \(F_{\mathrm{ovS}}\) decline, they obscure which physical mechanism is operating. This gap compromises our ability to determine if the freshening signals a future tipping point or merely a temporary fluctuation, directly undermining trust in model-based AMOC projections.
